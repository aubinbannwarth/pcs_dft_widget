...
From the characteristic function $f_A$ of the pitch class set $A$, we can compute the **discrete Fourier transform** (o[r DFT](https://en.wikipedia.org/wiki/Discrete_Fourier_transform)), $F_A$. This is another map fom  $\mathbb{Z}_{12}$ to $\mathbb{C}$, defined as follows:
...
The [circle of fifths](https://en.wikipedia.org/wiki/Circle_of_fifths) is a well known construct in music theory, used to study harmonic movement and the relationship between different keys. Two pitch classes $k_0$ and $k_1$ are said to be a fifth apart if 
$$k_1 - k_0 = 7 \mod 12.$$

If we define $k_0 = 0$, we can then define a sequence of consecutive fifths as follows:

$$k_{m+1} = (k_m + 7)\mod 12,\, m \in\{0, 10\}$$
Note that $k_{12}=k_0$ so there's no need to go further.
The sequence thus obtained is 
$$(k_0, k_1, ..., k_{11}) = (0, 7, 2, 9, 4, 11, 6, 1, 8, 3, 10, 5).$$

The **circle of fifths** maps each $k_m$ to a unique position on the unit circle according to the fomula
